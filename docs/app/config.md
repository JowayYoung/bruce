...
`命令配置`通过交互式命令生成并保存到终端内存中，当命令执行完毕就会被回收。

> 📦 构建应用 bruce-app b

选项|功能|形式|类型|默认|备注
:-:|:-:|:-:|:-:|:-:|-
**mode**|请选择环境模式|`选择`|`String`|`开发环境`|可选`dev开发环境`、`test测试环境`、`prod生产环境`
...
**deps**|请选择应用依赖|`选择`|`Array`|`[]`|提供`react`或`vue`相关开发依赖，如页面路由、状态管理和网络请求
...
配置文件使用`brucerc`命名，可选后缀为`js|json|yml`。不对配置做动态修改可用`json/yml`，否则使用`js`。一般情况下，应用最终上线会区分`测试环境`与`生产环境`，某些配置就需根据`mode`区分，此时只能使用`js`后缀了。
